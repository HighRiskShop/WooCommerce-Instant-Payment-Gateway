...
class HighRiskShop_Instant_Payment_Gateway_Paybis extends WC_Payment_Gateway {
    ...
    return false;
        ...
        $paybiscom_admin_wallet_address = isset($_POST[$this->plugin_id . $this->id . '_paybiscom_wallet_address']) ? sanitize_text_field( wp_unslash( $_POST[$this->plugin_id . $this->id . '_paybiscom_wallet_address'])) : '';

        // Check if wallet address starts with "0x"
        if (substr($paybiscom_admin_wallet_address, 0, 2) !== '0x') {
        ...
        if (strtolower($paybiscom_admin_wallet_address) === '0x3c499c542cef5e3811e1192ce70d8cc03d5c3359') {
            WC_Admin_Settings::add_error(__('Invalid Wallet Address: Please insert your USDC Polygon wallet address.', 'highriskshopgateway'));
            return false;
        }
...
$highriskshopgateway_paybiscom_response = wp_remote_get('https://api.highriskshop.com/control/convert.php?value=' . $highriskshopgateway_paybiscom_total . '&from=' . strtolower($highriskshopgateway_paybiscom_currency), array('timeout' => 30));
...
$highriskshopgateway_paybiscom_gen_wallet = wp_remote_get('https://api.highriskshop.com/control/wallet.php?address=' . $this->paybiscom_wallet_address .'&callback=' . urlencode($highriskshopgateway_paybiscom_callback), array('timeout' => 30));
...
if (is_wp_error($highriskshopgateway_paybiscom_gen_wallet)) {
 ...
 // Check if decoding was successful
    ...
    $order->add_meta_data('highriskshop_paybiscom_tracking_address', $highriskshopgateway_paybiscom_gen_addressIn, true);
    $order->add_meta_data('highriskshop_paybiscom_polygon_temporary_order_wallet_address', $highriskshopgateway_paybiscom_gen_polygon_addressIn, true);
    $order->add_meta_data('highriskshop_paybiscom_callback', $highriskshopgateway_paybiscom_gen_callback, true);
	$order->add_meta_data('highriskshop_paybiscom_converted_amount', $highriskshopgateway_paybiscom_final_total, true);
	$order->add_meta_data('highriskshop_paybiscom_expected_amount', $highriskshopgateway_paybiscom_reference_total, true);
	$order->add_meta_data('highriskshop_paybiscom_nonce', $highriskshopgateway_paybiscom_nonce, true);
    $order->save();
    } else {
...
function highriskshopgateway_paybiscom_change_order_status_callback( $request ) {
	...
	$highriskshopgateway_paybiscompaid_txid_out = sanitize_text_field($request->get_param('txid_out'));
	...
	// Verify nonce
    if ( empty( $highriskshopgateway_paybiscomgetnonce ) || $order->get_meta('highriskshop_paybiscom_nonce', true) !== $highriskshopgateway_paybiscomgetnonce ) {
	...
	$highriskshopgateway_paybiscomthreshold = 0.60 * $highriskshopgateway_paybiscomexpected_amount;
		if ( $highriskshopgateway_paybiscomfloatpaid_value_coin < $highriskshopgateway_paybiscomthreshold ) {
            ...
            $order->update_status('failed', __( 'Payment received is less than 60% of the order total. Customer may have changed the payment values on the checkout page.', 'highriskshop-instant-payment-gateway-paybis' ));
			/* translators: 1: Transaction ID */
            $order->add_order_note(sprintf( __( 'Order marked as failed: Payment received is less than 60%% of the order total. Customer may have changed the payment values on the checkout page. TXID: %1$s', 'highriskshop-instant-payment-gateway-paybis' ), $highriskshopgateway_paybiscompaid_txid_out));
		...
		/* translators: 1: Transaction ID */
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'highriskshop-instant-payment-gateway-paybis'), $highriskshopgateway_paybiscompaid_txid_out) );
