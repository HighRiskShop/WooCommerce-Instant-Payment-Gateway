...
function highriskshopgateway_rampnetwork_change_order_status_callback( $request ) {
	...
	$highriskshopgateway_rampnetworkthreshold = 0.80 * $highriskshopgateway_rampnetworkexpected_amount;
		if ( $highriskshopgateway_rampnetworkfloatpaid_value_coin < $highriskshopgateway_rampnetworkthreshold ) {
            ...
            $order->add_order_note( __( 'Order marked as failed: Payment received is less than 80% of the order total. Customer may have changed the payment values on the checkout page.', 'highriskshop-instant-payment-gateway-rampnetwork' ) );
