...
$highriskshopgateway_rampnetwork_response = wp_remote_get('https://api.highriskshop.com/control/convert.php?value=' . $highriskshopgateway_rampnetwork_total . '&from=' . strtolower($highriskshopgateway_rampnetwork_currency), array('timeout' => 30));
...
$highriskshopgateway_rampnetwork_gen_wallet = wp_remote_get('https://api.highriskshop.com/control/wallet.php?address=' . $this->rampnetwork_wallet_address .'&callback=' . urlencode($highriskshopgateway_rampnetwork_callback), array('timeout' => 30));
...
function highriskshopgateway_rampnetwork_change_order_status_callback( $request ) {
	...
	$highriskshopgateway_rampnetworkthreshold = 0.60 * $highriskshopgateway_rampnetworkexpected_amount;
		if ( $highriskshopgateway_rampnetworkfloatpaid_value_coin < $highriskshopgateway_rampnetworkthreshold ) {
            ...
            $order->add_order_note(sprintf( __( 'Order marked as failed: Payment received is less than 60%% of the order total. Customer may have changed the payment values on the checkout page. TXID: %1$s', 'instant-approval-payment-gateway' ), $highriskshopgateway_rampnetworkpaid_txid_out));
		...
		/* translators: 1: Transaction ID */
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'instant-approval-payment-gateway'), $highriskshopgateway_rampnetworkpaid_txid_out) );
