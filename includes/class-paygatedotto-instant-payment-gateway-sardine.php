...
if ($paygatedottogateway_sardineai_currency === 'USD') {
        $paygatedottogateway_sardineai_minimumcheck = $paygatedottogateway_sardineai_total;
		} else {
		
$paygatedottogateway_sardineai_minimumcheck_response = wp_remote_get('https://api.paygate.to/control/convert.php?value=' . $paygatedottogateway_sardineai_total . '&from=' . strtolower($paygatedottogateway_sardineai_currency), array('timeout' => 30));

if (is_wp_error($paygatedottogateway_sardineai_minimumcheck_response)) {
    // Handle error
...
$paygatedottogateway_sardineai_minimumcheck_body = wp_remote_retrieve_body($paygatedottogateway_sardineai_minimumcheck_response);
$paygatedottogateway_sardineai_minimum_conversion_resp = json_decode($paygatedottogateway_sardineai_minimumcheck_body, true);

if ($paygatedottogateway_sardineai_minimum_conversion_resp && isset($paygatedottogateway_sardineai_minimum_conversion_resp['value_coin'])) {
    // Escape output
    $paygatedottogateway_sardineai_minimum_conversion_total	= sanitize_text_field($paygatedottogateway_sardineai_minimum_conversion_resp['value_coin']);
    $paygatedottogateway_sardineai_minimumcheck = (float)$paygatedottogateway_sardineai_minimum_conversion_total;	
} else {
...
if ($paygatedottogateway_sardineai_minimumcheck < 30) {
