...
add_action('plugins_loaded', 'init_paygatedottogateway_utorg_gateway');

function init_paygatedottogateway_utorg_gateway() {
    if (!class_exists('WC_Payment_Gateway')) {
        return;
    }

class PayGateDotTo_Instant_Payment_Gateway_Utorg extends WC_Payment_Gateway {

    protected $icon_url;
    protected $utorgpro_wallet_address;

    public function __construct() {
        $this->id                 = 'paygatedotto-instant-payment-gateway-utorg';
        $this->icon = sanitize_url($this->get_option('icon_url'));
        $this->method_title       = esc_html__('Instant Approval Payment Gateway with Instant Payouts (utorg.pro)', 'instant-approval-payment-gateway'); // Escaping title
        $this->method_description = esc_html__('Instant Approval High Risk Merchant Gateway with instant payouts to your USDC POLYGON wallet using utorg.pro infrastructure', 'instant-approval-payment-gateway'); // Escaping description
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => esc_html__('Enable utorg.pro payment gateway', 'instant-approval-payment-gateway'), // Escaping label
                'default' => 'no',
            ),
            'title' => array(
    ...
    public function process_payment($order_id) {
        ...
        $paygatedottogateway_utorgpro_currency = get_woocommerce_currency();
		$paygatedottogateway_utorgpro_total = $order->get_total();
		$paygatedottogateway_utorgpro_nonce = wp_create_nonce( 'paygatedottogateway_utorgpro_nonce_' . $order_id );
...
if (is_wp_error($paygatedottogateway_utorgpro_gen_wallet)) {
	...
	$paygatedottogateway_utorgpro_wallet_decbody = json_decode($paygatedottogateway_utorgpro_wallet_body, true);

 // Check if decoding was successful
    if ($paygatedottogateway_utorgpro_wallet_decbody && isset($paygatedottogateway_utorgpro_wallet_decbody['address_in'])) {
        // Store the address_in as a variable
        $paygatedottogateway_utorgpro_gen_addressIn = wp_kses_post($paygatedottogateway_utorgpro_wallet_decbody['address_in']);
        $paygatedottogateway_utorgpro_gen_polygon_addressIn = sanitize_text_field($paygatedottogateway_utorgpro_wallet_decbody['polygon_address_in']);
		$paygatedottogateway_utorgpro_gen_callback = sanitize_url($paygatedottogateway_utorgpro_wallet_decbody['callback_url']);
		// Save $utorgproresponse in order meta data
    $order->add_meta_data('paygatedotto_utorgpro_tracking_address', $paygatedottogateway_utorgpro_gen_addressIn, true);
    $order->add_meta_data('paygatedotto_utorgpro_polygon_temporary_order_wallet_address', $paygatedottogateway_utorgpro_gen_polygon_addressIn, true);
    $order->add_meta_data('paygatedotto_utorgpro_callback', $paygatedottogateway_utorgpro_gen_callback, true);
	$order->add_meta_data('paygatedotto_utorgpro_converted_amount', $paygatedottogateway_utorgpro_final_total, true);
...
if (paygatedottogateway_is_checkout_block()) {
	...
	$woocommerce->cart->empty_cart();
        ...
        return array(
            'result'   => 'success',
            'redirect' => 'https://checkout.paygate.to/process-payment.php?address=' . $paygatedottogateway_utorgpro_gen_addressIn . '&amount=' . (float)$paygatedottogateway_utorgpro_final_total . '&provider=utorg&email=' . $paygatedottogateway_utorgpro_email . '&currency=' . $paygatedottogateway_utorgpro_currency,
        );
    }
...
function paygatedottogateway_utorgpro_change_order_status_rest_endpoint() {
    // Register custom route
    register_rest_route( 'paygatedottogateway/v1', '/paygatedottogateway-utorgpro/', array(
        'methods'  => 'GET',
        'callback' => 'paygatedottogateway_utorgpro_change_order_status_callback',
        'permission_callback' => '__return_true',
    ));
}
add_action( 'rest_api_init', 'paygatedottogateway_utorgpro_change_order_status_rest_endpoint' );

// Callback function to change order status
function paygatedottogateway_utorgpro_change_order_status_callback( $request ) {
    $order_id = absint($request->get_param( 'order_id' ));
	$paygatedottogateway_utorgprogetnonce = sanitize_text_field($request->get_param( 'nonce' ));
	$paygatedottogateway_utorgpropaid_txid_out = sanitize_text_field($request->get_param('txid_out'));
	...
	// Verify nonce
    if ( empty( $paygatedottogateway_utorgprogetnonce ) || $order->get_meta('paygatedotto_utorgpro_nonce', true) !== $paygatedottogateway_utorgprogetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'instant-approval-payment-gateway' ), array( 'status' => 403 ) );
    }

    // Check if the order is pending and payment method is 'paygatedotto-instant-payment-gateway-utorg'
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'paygatedotto-instant-payment-gateway-utorg' === $order->get_payment_method() ) {
        // Change order status to processing
		$order->payment_complete();
		/* translators: 1: Transaction ID */
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'instant-approval-payment-gateway'), $paygatedottogateway_utorgpropaid_txid_out) );
