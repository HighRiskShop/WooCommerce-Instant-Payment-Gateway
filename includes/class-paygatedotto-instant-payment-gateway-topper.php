...
if ($paygatedottogateway_topperpaycom_currency === 'USD') {
        $paygatedottogateway_topperpaycom_minimumcheck = $paygatedottogateway_topperpaycom_total;
		} else {
		
$paygatedottogateway_topperpaycom_minimumcheck_response = wp_remote_get('https://api.paygate.to/control/convert.php?value=' . $paygatedottogateway_topperpaycom_total . '&from=' . strtolower($paygatedottogateway_topperpaycom_currency), array('timeout' => 30));

if (is_wp_error($paygatedottogateway_topperpaycom_minimumcheck_response)) {
    // Handle error
...
$paygatedottogateway_topperpaycom_minimumcheck_body = wp_remote_retrieve_body($paygatedottogateway_topperpaycom_minimumcheck_response);
$paygatedottogateway_topperpaycom_minimum_conversion_resp = json_decode($paygatedottogateway_topperpaycom_minimumcheck_body, true);

if ($paygatedottogateway_topperpaycom_minimum_conversion_resp && isset($paygatedottogateway_topperpaycom_minimum_conversion_resp['value_coin'])) {
    // Escape output
    $paygatedottogateway_topperpaycom_minimum_conversion_total	= sanitize_text_field($paygatedottogateway_topperpaycom_minimum_conversion_resp['value_coin']);
    $paygatedottogateway_topperpaycom_minimumcheck = (float)$paygatedottogateway_topperpaycom_minimum_conversion_total;	
} else {
...
if ($paygatedottogateway_topperpaycom_minimumcheck < 10) {
