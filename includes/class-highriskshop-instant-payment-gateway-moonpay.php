...
class HighRiskShop_Instant_Payment_Gateway_Moonpay extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_description = esc_html__('Instant Approval High Risk Merchant Gateway with instant payouts to your USDC POLYGON wallet using moonpay.com infrastructure', 'highriskshopgateway'); // Escaping description
...
function highriskshopgateway_moonpaycom_change_order_status_callback( $request ) {
    ...
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'highriskshop-instant-payment-gateway-moonpay' === $order->get_payment_method() ) {
		...
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'highriskshop-instant-payment-gateway-moonpay'), $highriskshopgateway_moonpaycompaid_txid_out) );
