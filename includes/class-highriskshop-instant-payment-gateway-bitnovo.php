...
add_action('plugins_loaded', 'init_highriskshopgateway_bitnovo_gateway');

function init_highriskshopgateway_bitnovo_gateway() {
...
class HighRiskShop_Instant_Payment_Gateway_Bitnovo extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title       = esc_html__('Instant Approval Payment Gateway with Instant Payouts (bitnovo.com)', 'highriskshopgateway'); // Escaping title
        $this->method_description = esc_html__('Instant Approval High Risk Merchant Gateway with instant payouts to your USDC POLYGON wallet using bitnovo.com infrastructure', 'highriskshopgateway'); // Escaping description
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'type'    => 'checkbox',
                'label'   => esc_html__('Enable bitnovo.com payment gateway', 'highriskshopgateway'), // Escaping label
                'default' => 'no',
            ),
            'title' => array(
...
if (is_wp_error($highriskshopgateway_bitnovocom_gen_wallet)) {
    // Handle error
    wc_add_notice(__('Wallet error:', 'woocommerce') . __('Payment could not be processed due to incorrect payout wallet settings, please contact website admin', 'hrsbitnovocom'), 'error');
    return null;
} else {
 ...
 // Check if decoding was successful
    ...
    $order->save();
        ...
        wc_add_notice(__('Payment error:', 'woocommerce') . __('Payment could not be processed, please try again (wallet address error)', 'bitnovocom'), 'error');
...
function highriskshop_add_instant_payment_gateway_bitnovo($gateways) {
    $gateways[] = 'HighRiskShop_Instant_Payment_Gateway_Bitnovo';
    return $gateways;
}
add_filter('woocommerce_payment_gateways', 'highriskshop_add_instant_payment_gateway_bitnovo');
}
...
function highriskshopgateway_bitnovocom_change_order_status_callback( $request ) {
    ...
    if ( empty( $order_id ) ) {
        return new WP_Error( 'missing_order_id', __( 'Order ID parameter is missing.', 'highriskshop-instant-payment-gateway-bitnovo' ), array( 'status' => 400 ) );
    }
    ...
    if ( ! $order ) {
        return new WP_Error( 'invalid_order', __( 'Invalid order ID.', 'highriskshop-instant-payment-gateway-bitnovo' ), array( 'status' => 404 ) );
    }
    ...
    if ( empty( $highriskshopgateway_bitnovocomgetnonce ) || $order->get_meta('highriskshop_bitnovocom_nonce', true) !== $highriskshopgateway_bitnovocomgetnonce ) {
        return new WP_Error( 'invalid_nonce', __( 'Invalid nonce.', 'highriskshop-instant-payment-gateway-bitnovo' ), array( 'status' => 403 ) );
    }
    ...
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'highriskshop-instant-payment-gateway-bitnovo' === $order->get_payment_method() ) {
		...
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'highriskshop-instant-payment-gateway-bitnovo'), $highriskshopgateway_bitnovocompaid_txid_out) );
        // Return success response
        return array( 'message' => 'Order status changed to processing.' );
    } else {
        // Return error response if conditions are not met
        return new WP_Error( 'order_not_eligible', __( 'Order is not eligible for status change.', 'highriskshop-instant-payment-gateway-bitnovo' ), array( 'status' => 400 ) );
    }
}
?>