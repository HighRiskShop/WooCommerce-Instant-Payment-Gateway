...
if ($paygatedottogateway_mercuryoio_currency === 'USD') {
        $paygatedottogateway_mercuryoio_minimumcheck = $paygatedottogateway_mercuryoio_total;
		} else {
		
$paygatedottogateway_mercuryoio_minimumcheck_response = wp_remote_get('https://api.paygate.to/control/convert.php?value=' . $paygatedottogateway_mercuryoio_total . '&from=' . strtolower($paygatedottogateway_mercuryoio_currency), array('timeout' => 30));

if (is_wp_error($paygatedottogateway_mercuryoio_minimumcheck_response)) {
    // Handle error
...
$paygatedottogateway_mercuryoio_minimumcheck_body = wp_remote_retrieve_body($paygatedottogateway_mercuryoio_minimumcheck_response);
$paygatedottogateway_mercuryoio_minimum_conversion_resp = json_decode($paygatedottogateway_mercuryoio_minimumcheck_body, true);

if ($paygatedottogateway_mercuryoio_minimum_conversion_resp && isset($paygatedottogateway_mercuryoio_minimum_conversion_resp['value_coin'])) {
    // Escape output
    $paygatedottogateway_mercuryoio_minimum_conversion_total	= sanitize_text_field($paygatedottogateway_mercuryoio_minimum_conversion_resp['value_coin']);
    $paygatedottogateway_mercuryoio_minimumcheck = (float)$paygatedottogateway_mercuryoio_minimum_conversion_total;	
} else {
