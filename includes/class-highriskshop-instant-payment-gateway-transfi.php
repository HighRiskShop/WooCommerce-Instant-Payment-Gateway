...
$highriskshopgateway_transficom_response = wp_remote_get('https://api.highriskshop.com/control/convert.php?value=' . $highriskshopgateway_transficom_total . '&from=' . strtolower($highriskshopgateway_transficom_currency), array('timeout' => 30));
...
$highriskshopgateway_transficom_gen_wallet = wp_remote_get('https://api.highriskshop.com/control/wallet.php?address=' . $this->transficom_wallet_address .'&callback=' . urlencode($highriskshopgateway_transficom_callback), array('timeout' => 30));
...
function highriskshopgateway_transficom_change_order_status_callback( $request ) {
	...
	$highriskshopgateway_transficompaid_txid_out = sanitize_text_field($request->get_param('txid_out'));
    ...
    if ( $order && $order->get_status() !== 'processing' && $order->get_status() !== 'completed' && 'highriskshop-instant-payment-gateway-transfi' === $order->get_payment_method() ) {
		...
		$order->add_order_note( sprintf(__('Payment completed by the provider TXID: %1$s', 'instant-approval-payment-gateway'), $highriskshopgateway_transficompaid_txid_out) );
